...
export default function StatsKpiCards({ cards }: Props) {
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
      {cards.map((card) => (
        <div key={card.label} className="bg-[#1a1a2e] dark:bg-white dark:shadow-sm rounded-2xl p-5">
          <p className={`text-2xl font-bold ${card.color}`}>{card.value}</p>
          <p className="text-white dark:text-gray-900 text-sm font-medium mt-1">{card.label}</p>
