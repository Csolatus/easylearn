...
/** Liste tous les cours (optionnellement filtrés par école). */
export function useCourses(schoolId?: string) {
  ...
  const createCourse = useCallback(
    async (data: CourseCreate): Promise<CourseResponse> => {
  ...
  const updateCourse = useCallback(
    async (courseId: string, data: CourseUpdate): Promise<CourseResponse> => {
      const updated = await api.courses.update(courseId, data);
      setCourses(
        useCourseStore.getState().courses.map((c) => (c.id === courseId ? updated : c))
      );
      return updated;
    },
    [setCourses]
...
export function useCourse(courseId: string) {
  ...
  const createLesson = useCallback(
    async (data: LessonCreate): Promise<LessonResponse> => {
  ...
  const updateLesson = useCallback(
    async (lessonId: string, data: LessonUpdate): Promise<LessonResponse> => {
      const updated = await api.courses.lessons.update(courseId, lessonId, data);
      setLessons(
        useCourseStore.getState().lessons
